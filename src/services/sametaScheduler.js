...
function startSametaScheduler() {
    ...
    // 7:00 AM IST — morning motivation
    cron.schedule("0 7 * * *", runSametaJob, { timezone: TZ });
    // 6:00 PM IST — evening commute / after-work
    cron.schedule("0 18 * * *", runSametaJob, { timezone: TZ });

    logger.info("SametaCron", `Scheduled: 7:00 AM IST + 6:00 PM IST (${TZ})`);
}
